...
/// Игра по умолчанию — «101» (Кункен). С неё приложение начиналось, её
/// правило (вылет до 101) действует, когда партия начата без выбранной игры.
const String kDefaultGameId = 'g101';

/// Тип партии: явный тег из карты `gameTypes` или игра по умолчанию.
///
/// Тег пишется в карту-спутник при старте партии, и раньше партия без тега
/// (начата до тегирования, восстановлена из старого бэкапа, продолжена из
/// истории с потерянным тегом) выпадала из статистики, фильтра истории и
/// сравнения в аналитике — хотя фактически игралась по правилу «101».
String gameTypeIdFor(String gameId, Map<String, String> types) =>
    types[gameId] ?? kDefaultGameId;
